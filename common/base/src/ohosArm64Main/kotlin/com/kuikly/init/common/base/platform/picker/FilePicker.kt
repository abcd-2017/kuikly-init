...
/**
 * OHOS 文件选择器实现（通过 KNOI 调用 ETS 侧）
 *
 * 基于 @ohos.file.picker，由 ArkTS 侧实现具体逻辑。
 * 返回 JSON 字符串，Kotlin 侧手动解析为 PickedFile 列表。
 */
actual class FilePicker {
    ...
    actual suspend fun pickFile(mimeType: String, allowMultiple: Boolean): List<PickedFile> {
        return try {
            val result = service?.pickFile(mimeType, allowMultiple) ?: return emptyList()
            parseResult(result)
        } catch (e: Exception) {
            emptyList()
        }
    ...
    actual suspend fun pickImage(allowMultiple: Boolean): List<PickedFile> {
        return try {
            val result = service?.pickImage(allowMultiple) ?: return emptyList()
            parseResult(result)
        } catch (e: Exception) {
            emptyList()
        }
    ...
    actual suspend fun pickDocument(allowMultiple: Boolean): List<PickedFile> {
        return try {
            val result = service?.pickDocument(allowMultiple) ?: return emptyList()
            parseResult(result)
        } catch (e: Exception) {
            emptyList()
        }
     ...
     */
    private fun parseResult(jsonStr: String): List<PickedFile> {
        try {
            ...
            for (obj in objects) {
                ...
                val mimeType = extractStringValue(cleanObj, "mimeType")
                files.add(PickedFile(path, name, size, mimeType))
            }
            return files
        } catch (e: Exception) {
            return emptyList()
