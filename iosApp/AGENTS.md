...
2. **页面控制器**：`KuiklyRenderViewController` 管理单个 Kuikly 页面的生命周期
3. **原生桥接**：`HRBridgeModule` 实现 Kotlin 侧声明的桥接方法
4. **路由处理**：`KRRouterHandler` 管理页面导航栈（push/pop）
5. **图片扩展**：`KuiklyRenderComponentExpandHandler` 使用 SDWebImage 加载图片

## 详细目录结构

```
iosApp/
├── Podfile                        # CocoaPods 依赖：OpenKuiklyIOSRender 2.7.0, SDWebImage
...
└── iosApp/
    ...
    ├── ContentView.swift          # 📌 根视图：加载 "router" 页面
    ├── KuiklyRenderViewPage.swift # 📌 UIViewControllerRepresentable 包装器
    ├── Info.plist                 # 应用配置
    ...
    └── KuiklyExpand/              # 📌 Kuikly 扩展层
        ...
        │   ├── KRRouterHandler.m         # 📌 路由处理器：openPage / closePage
        │   ├── KuiklyRenderComponentExpandHandler.h  # 组件扩展头文件
        │   └── KuiklyRenderComponentExpandHandler.m  # 📌 组件扩展：SDWebImage 加载 + 颜色解析
        └── Modules/                      # 原生桥接模块
...
### HRBridgeModule
- 继承 `KRBaseModule`
- **已实现**：`copyToPasteboard`（UIPasteboard）、`log`（NSLog）
- **待实现**：与 Android 侧对齐的其他桥接方法

## 依赖 (CocoaPods)

```
OpenKuiklyIOSRender ~> 2.7.0   # Kuikly iOS 渲染引擎
