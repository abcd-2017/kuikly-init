...
@Page(PAGE_NAME)
internal class ImageAdapterBenchmarks : ComposeContainer() {
...
private fun Content() {
    ...
    val edgeInsets = EdgeInsets(32f, 32f, 32f, 32f)
    ...
    Row {
        ...
        Box(Modifier.size(boxSize)) {
            Image(
                ...
                modifier = Modifier
                    .size(boxSize)
                    .setProp("capInsets", edgeInsets.toString())
            )
            Box(Modifier.padding(32.dp).fillMaxSize().background(Color.Green.copy(alpha = 0.5f)))
        }
    }
    Check("intrinsicSize测试", result1)
    Check("Image测试, 查看蓝色方块是否与图片重合")
    Check("drawImage测试, 查看红色方块是否与图片重合")
    Check("capInsets测试, 查看绿色方块是否与中心黑色区域重合")
    ...
    LaunchedEffect(assetsImage.width, assetsImage.height) {
        if (assetsImage.width != assetsIntrinsicSize2.width || assetsImage.height != assetsIntrinsicSize2.height) {
            ...
            result22 =
                assetsImage.width == STANDARD_SIZE.width.toInt() && assetsImage.height == STANDARD_SIZE.height.toInt()
        }
    }
    Row {
        Box(Modifier.size(boxSize)) {
...
private val sample: DrawableResource by lazy(LazyThreadSafetyMode.NONE) {
    DrawableResource(ImageUri.pageAssets("sample.png").toUrl(PAGE_NAME))
}